...
function generateDayContainers(duration, startDate, itinerary_days = []) {
  ...
  document.querySelectorAll(".add-time-btn").forEach((button) => {
    for (let i = 0; i < itinerary_days.length; i++) {
      ...
      addTimeSlot(
        dayNumber,
        itinerary_days[i].start_time,
        itinerary_days[i].end_time,
        itinerary_days[i].activity
      );
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  startDateInput.addEventListener("change", computeItineraryDuration);
  endDateInput.addEventListener("change", computeItineraryDuration);
});
