...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result && $result['has_itinerary'] == 1) {
        ...
        for ($day_number = 1; $day_number <= $duration_days; $day_number++) {
            ...
            error_log("TIME RANGES: " . $time_ranges);
            error_log("ACTIVITIES: " . $activities);
            ...
            if (!empty($time_ranges) && !empty($activities)) {
                ...
                for ($i = 0; $i < count($time_ranges); $i++) {
                    ...
                    if (isset($time_ranges[$i * 2]) && isset($time_ranges[($i * 2) + 1]) && isset($activities[$i])) {
                        ...
                        $sql = "INSERT INTO customized_intinerary_days (customized_initerary_id, day_number, start_time, end_time, activity) 
                                VALUES (:customized_initerary_id, :day_number, :start_time, :end_time, :activity)";
                        $stmt = $pdo->prepare($sql);
                        $stmt->execute([
                            ':customized_initerary_id' => $updatedItineraryId,
            ...
            if (!empty($time_ranges) && !empty($activities)) {
                for ($i = 0; $i < count($time_ranges); $i++) {
                    ...
                    if (isset($time_ranges[$i * 2]) && isset($time_ranges[($i * 2) + 1]) && isset($activities[$i])) {
                        ...
                        error_log(message: "INSERT INTO TABLE");
